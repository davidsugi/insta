...
<div class="row">
	<div class="col-xs-12">
		<div class="box box-info">
			...
			<div class="col-sm-2"> </div>
	Perintah:<a class="btn btn-success buttoned" href="{{ action('DetailListController@createid', $res->id) }}" role="button"><span class="fa fa-plus"></span> Add</a>
...
{{-- 			@foreach ($row->toArray() as $column)
				...
				@endforeach --}}
				<td>{{ $row->id }}</td>
				<td><a href="{{ route('detaillist.show', $row->id) }}">{{ $row->username }}<a/></td>
				<td>{{ $row->name }}</td>
				
				<td><a class="btn btn-primary buttoned" href="{{ action('DetailListController@edit', $row->id) }}" role="button">
				<span class="fa fa-pencil"></span></a>

				
				<a class="btn btn-danger buttoned" href="{{ action('DetailListController@destroy', $row->id) }}" role="button"><span class="fa fa-trash-o"></span></a>
				</td>
			</tr>
